...
public class RectangleFactory {
    public static ComplexCommand rectangleCommand(Job2dDriver driver, int x, int y, int width, int length){
        ...
        command.addCommand(new OperateToCommand(driver, x + length, y));
        command.addCommand(new OperateToCommand(driver, x + length, y + width));
        command.addCommand(new OperateToCommand(driver, x, y + width));
